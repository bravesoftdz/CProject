...
begin
  ...
  if (_Timers.ItemList.Count * 50) > self.ClientHeight then
    scrollTimerList.Height := self.ClientHeight
  else
    scrollTimerList.Height := (_Timers.ItemList.Count * 50);
  ...
  begin
    ...
    begin
      case oTimerInfo.FStatus of
        tsReady: frameTimer1.txtTime.TextSettings.FontColor := COLOR_SCALE_READY;
        tsCooking: frameTimer1.txtTime.TextSettings.FontColor := COLOR_SCALE_COOKING;
        tsOverCooking: frameTimer1.txtTime.TextSettings.FontColor := COLOR_SCALE_OVERCOOKING;
      end;
    end;
