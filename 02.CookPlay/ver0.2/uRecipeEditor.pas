...
type
  ...
  private
    ...
    procedure CallbackMyRecipeListResult(aResultList: TStringList);
    ...
    procedure OnCallbackDragNDrop(AResultList: TStringList=nil);
...
uses cookplay.statusBar, uRecipeCategory, ClientModuleUnit,
  System.Math, uList, cookplay.S3, uListDragNDrop;
{$R *.fmx}
...
begin
  ...
  if FRecipe.Serial = NEW_RECIPE then
    AddStep(layoutStepBody, NEW_STEP) // 새로운 Step, Step Serial은 -1로 세팅
...
begin
  ...
  begin
    frmS3.LoadImageFromS3(BUCKET_RECIPE, TImage(Sender).TagString, TImage(Sender));
...
procedure TfrmRecipeEditor.OnCallbackDragNDrop(AResultList: TStringList);
var
  i, x, y: integer;
  str: string;
begin
...
//------------------------------------------------------------------------------
procedure TfrmRecipeEditor.CallbackMyRecipeListResult(aResultList: TStringList);
var
  aStep: TStepInfo;
begin
...
begin
  ...
  begin
    ...
    aIngredient.Serial := NEW_INGREDIENT;
    ...
    case it of
      itIngredient, itSeasoning:
        begin
          ...
          imgScale.OnMouseDown := Step.OnImageMouseDown;
          begin
...
begin
  AddStep(layoutStepBody, NEW_STEP);
end;
...
begin
  ...
  nRecipeSerial := NEW_RECIPE;
  nStepSerial := NEW_STEP;
  nIngredientSerial := NEW_INGREDIENT;
  ...
  try
    ...
    if nRecipeSerial = NEW_RECIPE then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if nIngredientSerial = NEW_INGREDIENT then
...
//       (LFocusRect.Bottom > FKBBounds.Top) then
    ...
    begin
      ...
      layoutScrollBottom.Height := self.ClientHeight - FKBBounds.Top - 120;
...
procedure TStepInfo.OnImageMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Single);
begin
  // 재료 수정중이면
  if CanMovingIngredient then
...
//    TLayout(FMovingLayout.parent).Root.Captured := TLayout(FMovingLayout.parent);
    layoutIngredient.Root.Captured := layoutIngredient;

//    frmRecipeEditor.layoutStepBody.Root.Captured := frmRecipeEditor.layoutStepBody;
