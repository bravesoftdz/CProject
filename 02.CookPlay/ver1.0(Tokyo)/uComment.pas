...
uses
  ...
  FMX.Memo, FMX.Ani, System.ImageList, FMX.ImgList, System.Actions, FMX.ActnList;
...
type
  TfrmComment = class(TForm)
    ...
    actionModifyComment: TAction;
    actionDeleteComment: TAction;
    ...
    procedure layoutCloseClick(Sender: TObject);
    procedure actionModifyCommentExecute(Sender: TObject);
    procedure actionDeleteCommentExecute(Sender: TObject);
  private
    { Private declarations }
    FCallBackBoolRefFunc: TCallbackBoolRefFunc;
    FModified: Boolean;
...
implementation
uses cookplay.StatusBar, ClientModuleUnit, cookplay.S3, uMenuBottom;
...
procedure TfrmComment.layoutCloseClick(Sender: TObject);
begin
  CloseForm;
end;
...
var
  i: integer;
begin
  FCommentList := CM.GetComment(FCommentType, FSerial);
  ...
  frmMenuBottom.Init(aList,
    ...
    begin
      ...
      begin
        ...
        if nIndex = 0 then
          actionModifyComment.Execute
        else if nIndex = 1 then
          actionDeleteComment.Execute;
      end;
    end
  );
...
procedure TfrmComment.actionDeleteCommentExecute(Sender: TObject);
begin
  HideMenu;
  ...
  MessageDlg( '댓글을 삭제하시겠습니까?', TMsgDlgType.mtConfirmation, [TMsgDlgBtn.mbYES, TMsgDlgBtn.mbNo], 0,
    ...
    begin
      ...
      begin
        if CM.DeleteComment(FCommentType, Comments(FSelectedComment).Serial) then
        begin
          ClearMemo;
...
procedure TfrmComment.actionModifyCommentExecute(Sender: TObject);
begin
  HideMenu;
  ShowModifyRectangle;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if recModify.Visible then
      begin
        if CM.UpdateComment(FCommentType, Comments(FSelectedComment).Serial, aComment) then
        begin
      ...
      begin
        if CM.InsertComment(FCommentType, FSerial, CM.memUserSerial.AsLargeInt, aComment) then
        begin
          ClearMemo;
...
function TfrmComment.Comments(aIndex: integer): TCommentInfo;
begin
  if Assigned(FCommentList) and (FCommentList.Count > 0) then
    result := TCommentInfo(FCommentList.Objects[aIndex])
  else
    result := nil;
end;
...
begin
  ...
  if FModified and Assigned(FCallbackBoolRefFunc) then
    FCallbackBoolRefFunc(True);

  FCallbackBoolRefFunc := nil;
end;
