  ...
  begin
    ...
    begin
      ...
      if nNewLastID = -1 then
        raise Exception.Create('Ingredient 시리얼을 읽어올 수 없습니다');

      // Method Table 에 링크된 Ingredient serial 을 Update 한다
...
begin
  ...
  begin
    ShowMessage('삭제할 재료가 없습니다!');
    Exit;
  end;

  if memIngredientStateCur.AsString = DS_DELETE then
  begin
    ShowMessage('이미 삭제된 재료 입니다!');
    Exit;
  end;

  if dmDB.FindSerial('RecipeIngredient_Serial', memIngredientSerial.AsLargeInt, memMethod) then
  begin
    ShowMessage('해당재료가 방법에 사용되어서 삭제할 수 없습니다!');
    Exit;
  end;

  if MessageDlg('삭제하시겠숩니까?', mtConfirmation, [mbYes, mbNo], 0, mbYes) = mrYes then
  begin
    try
      memIngredient.Edit;
...
begin
  if memIngredient.RecordCount <= 0 then
  begin
    ShowMessage('수정할 재료가 없습니다!');
    Exit;
  end;

  if memIngredientStateCur.AsString = DS_DELETE then
  begin
    ShowMessage('삭제된 재료입니다!');
    Exit;
  end;
